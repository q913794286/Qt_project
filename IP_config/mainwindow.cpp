...
void MainWindow::button_write()
{
    ...
    {
        QMessageBox::information(this,  QStringLiteral("设置完成"),QStringLiteral("请填刷卡获取目标IP以及端口！"));
    }
    else
    {
  /*****设置参数***************/
...
//    Server_udpSocket->writeDatagram(QVariant(ui->textEdit_write->toPlainText()).toByteArray(),QHostAddress::LocalHost,5000);//向5000端口写数据
    udpSocket->writeDatagram(cmd,1,send_Ip,send_Port);//向5000端口写数据
...
//    cmd1 = {0xA1,0x00};
    ...
    {
    //raly1 activting
    udpSocket->writeDatagram(cmd1,2,send_Ip,send_Port);//向5000端口写数据
    }
}
void MainWindow::raly2_send()
{
    ...
    {
    //raly2 activting
    udpSocket->writeDatagram(cmd2,2,send_Ip,send_Port);//向5000端口写数据
    }
}
